...
export default function SelectModeratorClient({
  conversations,
}: {
  conversations: IHistoryAdminModeratorConv[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((conv) =>
      (conv.participants.firstName + " " + conv.participants.lastName)
        .toLowerCase()
    ...
    <div className="flex flex-1 flex-col gap-4">
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 p-2 md:grid-cols-2 lg:grid-cols-3 lg:p-4">
          {filtered.map((conv) => (
            <Link
              href={`/dashboard/history/admin-moderator/chat/${conv.id}`}
              key={conv.id}
            >
              <TeamCustomCardView {...conv.participants} />
            </Link>
          ))}
        </div>
      ) : (
