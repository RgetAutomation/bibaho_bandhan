...
export default function SelectBrideClient({
  conversations,
}: {
  conversations: IHistoryTeamUserConv[];
}) {
    ...
    <div className="flex flex-1 flex-col gap-4">
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 p-2 md:grid-cols-2 lg:grid-cols-3 lg:p-4">
          {filtered.map((conv) => (
            <Link
              href={`/dashboard/history/chats/team/${conv.id}?path=moderator`}
              key={conv.id}
            >
              <TeamCustomCardView {...conv.user} />
            </Link>
          ))}
        </div>
      ) : (
