...
import { deleteTeamUserConversationById } from "@/action/conversation";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";
import Link from "next/link";

export default function TeamUserChatClient({
  conversationId,
  conversation,
  path = "admin",
}: {
  conversationId: string;
  conversation: ITeamUserConversation | null;
  ...
  const maleUser = conversation?.user;
  const teamUser = conversation?.team;
  ...
  const handleDeleteConversation = async (conversationId: string) => {
    try {
      const response = await deleteTeamUserConversationById(
        conversationId || ""
      );

      if (response.success) {
        const pathHistory =
          path === "admin" ? "admin-groom" : "moderator-bride";
        router.push(`/dashboard/history/${pathHistory}`);
      ...
      <div className={"flex items-center justify-between"}>
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button className="me-3">Delete Conversation</Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <div key={date}>
                  ...
                  {messages.map((msg) => {
                    ...
                    return (
                      <Link
                        key={msg.id}
                        href={`/dashboard/history/chats/team/delete/${msg.id}`}
                        className={`mb-3 flex ${
