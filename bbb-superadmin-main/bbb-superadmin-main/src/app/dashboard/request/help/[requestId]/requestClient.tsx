...
import { IHelpRequestSingle } from "@/components/interface/IHelpRequest";
...
export default function RequestClientPage({
  ...
  const [helpRequest, setHelpRequest] = useState<IHelpRequestSingle | null>(null);
  ...
  const fetchData = useCallback(() => {
    setIsLoading(true);
    getSingleHelpRequestById(requestId)
      .then((data) => {
        setHelpRequest(data);
        setError(null);
        form.reset({
  ...
  if (error) {
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center">
        <div className="bg-red-100 text-red-800 p-4 rounded-md text-center max-w-sm">
           <h2 className="text-xl font-bold mb-2">Failed to load request</h2>
           <p>{errorMessage}</p>
        </div>
      </div>
    );
...
function AdminHelpChatBox({ requestId, currentUserId, isResolved }: { requestId: string, currentUserId: string, isResolved: boolean }) {
  const [messages, setMessages] = useState<IHelpRequestMessage[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const data = await getHelpRequestMessages(requestId);
      setMessages(data);
    } catch (error) {
  ...
  const handleSendMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSending(true);
    try {
      await sendAdminHelpMessage(requestId, content);
      setLastReadId(null);
      await fetchMessages();
      form.reset();
    } catch (error) {
      ...
      const errorMessage = isAxiosError(error)
        ? (error.response?.data?.message || error.message || "Network error - check API server")
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg: IHelpRequestMessage, index: number) => {
            ...
            const senderName = msg.teamSender
              ? `#${getIdPrefix(msg.teamSender.internalId, msg.teamSender.role as "ADMIN" | "MODERATOR" | "GHOTOK" | "SUPERADMIN")}`
              : "Support";
      ...
      {isResolved ? (
        ...
        <form onSubmit={handleSendMessage} className="p-3 border-t bg-background flex gap-2">
          ...
          <Button 
            ...
            disabled={isSending}
          >
            {isSending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5 ml-[-2px]" />}
          </Button>
        </form>
      )}
    </div>
