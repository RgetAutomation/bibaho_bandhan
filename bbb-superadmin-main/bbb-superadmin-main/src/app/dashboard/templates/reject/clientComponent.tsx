...
import {
  deleteRejectMessageTemplate,
  updateRejectMessageTemplateStatus,
} from "@/action/templates";

export default function RejectTemplateCardView({
  id,
  name,
  content,
  isActive,
...
function TemplateClientComponent({
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const response = await deleteRejectMessageTemplate(id);
      if (response.success) {
  ...
  const handleActiveOrDeactive = async () => {
    setActivating(true);
    try {
      const response = await updateRejectMessageTemplateStatus(id, !status);
      if (response.success) {
        toast.success(response.message);
        router.refresh();
      } else {
    ...
    <div className="flex items-center justify-between pt-3">
      ...
      <div className="flex gap-1">
        <Button variant={"secondary"} size={"icon"}>
          <Link href={`/dashboard/templates/reject/${id}`}>
            <Edit />
          </Link>
        </Button>
