...
import { reportTeamReply } from "@/action/reported";
...
export default function ReportedUserClient({ reportId }: { reportId: string }) {
  ...
  async function onSubmit(data: z.infer<typeof replySchema>) {
    setLoading(true);
    try {
      const response = await reportTeamReply(reportId, data.reply);
      if (response.success) {
  ...
  return (
    <div className="bg-muted-foreground/10 rounded-md p-4">
      <Form {...form}>
