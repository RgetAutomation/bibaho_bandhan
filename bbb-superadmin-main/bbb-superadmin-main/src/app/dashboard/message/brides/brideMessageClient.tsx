...
import { getAllBridesConversations } from "@/action/conversation";
...
export default function BrideMessageClient({
  ...
  const { userSAConversationIds: brideConversationIds, removeUserSAConversation: removeBrideConversation } =
    useNotificationStore();

  const [query, setQuery] = useState("");

  const [brides, setBrides] = useState<ITeamUserConversations[]>([]);
  ...
  useEffect(() => {
    const fetchBrides = async () => {
      try {
        const data = await getAllBridesConversations();
        setBrides(data as ITeamUserConversations[]);
      } catch (err) {
        setError(err);
      } finally {
        setIsLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    return brides?.filter((bride: ITeamUserConversations) =>
      bride.user.lastName.toString().includes(query.toLowerCase())
    );
  }, [query, brides]);
  ...
  }, [teamUserSocket, isConnected, joinConversation, brides]);
    ...
    <div className="flex flex-1 flex-col gap-4">
      ...
      {filtered && filtered.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 p-2 md:grid-cols-2 lg:grid-cols-3 lg:p-4">
          {filtered.map((bride: ITeamUserConversations) => (
            <Link
              href={`/dashboard/chat/${bride.id}`}
              key={bride.user.id}
              onClick={() => {
