...
import { IMatchedUsers } from "@/components/interface/IUsers";
...
export default function PaidMatchingGroomClient({
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    } else if (sortBy === "ghotok-active") {
      return [
        ...filtered.filter(
          (user) => user.isGhotokOwned && user.isExipired === false
        ),
      ];
    } else if (sortBy === "ghotok-expired") {
      return [
        ...filtered.filter(
          (user) => user.isGhotokOwned && user.isExipired === true
        ),
    ...
    <div className="flex flex-1 flex-col">
      <div className="flex flex-col items-center justify-between gap-4 p-4 md:flex-row md:p-6 lg:p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-800 md:text-2xl lg:text-3xl dark:text-gray-100">
            Paid Matching ({filteredAndSorted?.length})
          </h1>
        </div>
          ...
          <Select onValueChange={setSortBy} defaultValue="all">
            ...
            <SelectContent>
              ...
              <SelectItem value="ghotok-active">
                Ghotok Active Matched
              </SelectItem>
              <SelectItem value="ghotok-expired">
                Ghotok Expired Matched
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="flex flex-1 flex-col">
        {filteredAndSorted && filteredAndSorted.length > 0 ? (
          <div className="grid w-full grid-cols-1 gap-4 space-y-4 px-2 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-4">
            <EpmptyList
              ...
              subtitle="We could not find any groom that matches your search."
            />
          </div>
        )}
      </div>
