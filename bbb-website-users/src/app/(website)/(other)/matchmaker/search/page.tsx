...
const searchGhotokRequestSchema = z.object({
  ...
  phone: z
    ...
    .optional()
    .refine(
      (val) => !val || /^[0-9]{10}$/.test(val), // only if not empty
...
export default function SearchGhotokStatus() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [respose, setRespose] = useState<DataRespose>();

  const form = useForm<z.infer<typeof searchGhotokRequestSchema>>({
    resolver: zodResolver(searchGhotokRequestSchema),
    defaultValues: {
      requestId: "",
      phone: "",
      email: "",
  ...
  async function onSubmit(data: z.infer<typeof searchGhotokRequestSchema>) {
    setLoading(true);
    ...
    try {
      const respone = await axios.post<AxiosResponse<string>>(
        `${MAIN_API_URL}/app/ghotok/join/search`,
        data,
      );

      if (respone.data.success) {
        router.push(`/matchmaker/status/${respone.data.data}`);
      }
    ...
    <div className="flex flex-col flex-1 items-center justify-center px-4 py-10">
      <div className="md:max-w-md flex flex-col border border-border shadow-md rounded-2xl p-8 w-full max-w-lg bg-card gap-4">
        ...
        <p className="text-sm text-muted-foreground text-center">
          Fill in request ID or mobile number or email to search request status.
        </p>
