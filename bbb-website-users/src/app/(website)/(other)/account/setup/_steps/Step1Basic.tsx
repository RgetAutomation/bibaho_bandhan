...
export default function Step1Basic({ onComplete }: { onComplete: () => void }) {
  const [loading, setLoading] = useState(false);
  const setData = useUpdatingProfileStore((state) => state.setData);
  const stateData = useUpdatingProfileStore();
  const createAccountState = useCreateAccountStore(); // Initial signup info
  ...
  const form = useForm<any>({
    ...
    defaultValues: {
      ...
      children: stateData.children || 0,
      childrenLivingWith: stateData.childrenLivingWith || "",
      motherTongue: stateData.motherTongue || "",
      spokenLanguages: stateData.spokenLanguages || "",
      emailId: stateData.emailId || createAccountState.email || "",
      phoneNumber: stateData.phoneNumber || createAccountState.mobile || "",
      whatsappNumber: stateData.whatsappNumber || "",
      alternatePhone: stateData.alternatePhone || "",
      relationshipWithBrideGroom: stateData.relationshipWithBrideGroom || "",
    },
  });
  ...
  const age = dobValue ? differenceInYears(new Date(), new Date(dobValue)) : null;
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <StepActions loading={loading} showBack={false} submitLabel="Save & Continue" />
        </form>
      </Form>
    </div>
  );
