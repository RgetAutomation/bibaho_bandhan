...
import { getPaidGrooms } from "@/actions/getGrooms";
...
export default function PaidGroomClient() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["getAllPaidGrooms"],
    queryFn: () => getPaidGrooms(),
  ...
  if (error) {
    const errorMessage = isAxiosError(error)
      ? error.response?.data?.message || "Failed to load paid grooms"
    ...
    <div className="flex min-h-screen w-full flex-1 flex-col md:min-h-min">
      ...
      {data && data.length > 0 ? (
        <div className="grid w-full grid-cols-1 gap-4 p-4 md:grid-cols-2 lg:grid-cols-3">
          {data.map((groom: Groom) => (
            <GroomComponent
              key={groom.id}
              groom={groom}
              groomBadge={GroomBadgeType.PAID}
              canReport={true}
            />
          ))}
        </div>
        ...
        <div className="flex min-h-screen w-full flex-1 items-center justify-center md:min-h-min">
          <ContentNotFound
            title={"No paid grooms found"}
            description={
              "We could not find any paid grooms. Please try again later."
            }
          />
        </div>
      )}
