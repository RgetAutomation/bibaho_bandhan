...
export default function GhotokGroomsClientPage() {
  const [query, setQuery] = useState("");
  const [page, setPage] = useState(1);
  const limit = 10;

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["getAllGhotokGrooms", page],
    queryFn: () => getAllGhotokGrooms(page, limit),
    ...
    <div className="relative flex w-full flex-1 flex-col">
      <div className="bg-card flex w-full flex-col justify-between gap-4 border p-4 md:flex-row md:p-6">
        ...
        <div className={"flex items-center justify-between gap-2"}>
          ...
          />
          <Button asChild>
            <Link href="/dashboard/ghotok/users/new?gender=MALE">
              Create User
            </Link>
          </Button>
        </div>
