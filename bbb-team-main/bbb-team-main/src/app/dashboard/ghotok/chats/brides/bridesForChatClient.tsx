...
import { getAllGhotokBridesForChat } from "@/actions/ghotok";
import ApiErrorPage from "@/components/apiErrorPage";
import ContentNotFound from "@/components/contentNotFound";
import { IGhotokUsersForChat } from "@/components/interface/ghotok/IUsersForChat";
...
export default function BridesForChatClient() {
  const { brideConversationIds } = useGhotokNotificationStore();
  const [page, setPage] = useState(1);
  const limit = 10;

  const { data, isLoading, error } = useQuery({
    queryKey: ["getAllGhotokBridesForChat", page],
    queryFn: () => getAllGhotokBridesForChat(page, limit),
  ...
  if (error) {
    const errorMessage = isAxiosError(error)
      ? error.response?.data?.message || "Failed to load brides"
      : "Something went wrong. Please try again later.";
    return (
      <div className={"flex w-full flex-1 items-center justify-center"}>
        <ApiErrorPage
          title={"Failed to load brides"}
      ...
      {brides && brides.length > 0 ? (
        ...
        >
          <div className="grid w-full grid-cols-1 gap-4 p-2 md:grid-cols-2 lg:grid-cols-3 lg:p-4">
            {brides.map((bride) => (
              <Link
                href={`/dashboard/ghotok/chats/brides/${bride.id}`}
                key={bride.id}
              >
                <UserCardView
                  key={bride.id}
                  user={bride}
                  showBadge={brideConversationIds
