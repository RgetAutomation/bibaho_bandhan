...
import {
  getSingleReportedGhotokUsers,
  getSingleReportedGroom,
} from "@/actions/getReportedUsers";
...
export default function ReportedSingleGroom({ userId }: { userId: string }) {
  const {
    ...
    queryKey: ["getSingleReportedGhotokUsers", userId],
    queryFn: () => getSingleReportedGhotokUsers(userId),
  ...
  return (
    <Card className="m-4 max-w-xl rounded-2xl shadow-md">
      ...
      <CardContent className="space-y-4">
        {/* User Info */}
        <Link href={`/dashboard/ghotok/profile/public/${report?.user.id}`}>
